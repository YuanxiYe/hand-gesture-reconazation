...
class ModelTraining:
    ...
    def usePretrainedModel(self, model_path):
        """
        'usePretrainedModel()' is used to set to use the pretrained model
        for the training instance object .
        :return:
        """
        self.__pretrained_model_path = model_path
    ...
    def setModelTypeAsVgg(self):
        """
         'setModelTypeAsVgg()' is used to set the model type to the Vgg model
                for the training instance object .
        :return:
        """
    ...
    def trainModel(self, num_objects, num_experiments=200, enhance_data=False, batch_size = 32, initial_learning_rate=1e-3, show_network_summary=False, training_image_size = 224):
        ...
        if (self.__modelType == "squeezenet"):
            if (self.__use_pretrained_model == False):
                model = SqueezeNet(weights="custom", num_classes=num_classes, model_input=image_input)
            else:
                model = SqueezeNet(weights="trained", num_classes=num_classes, model_input=image_input, model_path = self.__pretrained_model_path)
        elif (self.__modelType == "resnet"):
            if (self.__use_pretrained_model == False):
                model = ResNet50(weights="custom", num_classes=num_classes, model_input=image_input)
            else:
                model = ResNet50(weights="trained", num_classes=num_classes, model_input=image_input, model_path = self.__pretrained_model_path)
        elif (self.__modelType == "inceptionv3"):
            if (self.__use_pretrained_model == False):
                model = InceptionV3(weights="custom", classes=num_classes, model_input=image_input)
            else:
                model = InceptionV3(weights="trained", classes=num_classes, model_input=image_input, model_path = self.__pretrained_model_path)
        elif (self.__modelType == "densenet"):
            if (self.__use_pretrained_model == False):
                model = DenseNetImageNet121(weights="custom", classes=num_classes, model_input=image_input)
            else:
                model = DenseNetImageNet121(weights="trained", classes=num_classes, model_input=image_input, model_path = self.__pretrained_model_path)
        elif (self.__modelType == "vgg"):
            if (self.__use_pretrained_model == False):
                model = Vgg16(weights="custom", num_classes=num_classes, model_input=image_input)
            else:
                model = Vgg16(weights="trained", num_classes=num_classes, model_input=image_input, model_path = self.__pretrained_model_path)
        ...
        * prediction_speed (optional); Acceptable values are "normal", "fast", "faster" and "fastest"
        ...
        if (self.__modelLoaded == False):
            ...
            if (self.__modelType == ""):
                ...
                from .custom_utils import decode_predictions
                try:
                    model = Vgg16(model_path=self.modelPath, weights="trained", model_input=image_input, num_classes=self.numObjects)
                    self.__model_collection.append(model)
                    self.__modelLoaded = True
                except:
                    raise ValueError("You have specified an incorrect path to the ResNet model file.")
